...
var RESCODE_TOO_MANY_IMAGES = '41008';
var RESCODE_ALL_MENUID_INVALID = '41020';
var RESCODE_READ_CACHE_FAILED = '41025';
...
var DEFAULT_BOOL_GET_DETAIL_USER_INFO = false;           //默认不获取当前用户详细信息
...
function jsonData() {
    ...
    this.stopRecord = '{"localId":"SXVOICE_20160606173717"}';
    this.uploadVoice = '{"serverId":"http://106.120.205.116:680/M00/02/FB/rBAICldVRQuAEm58AAAYgcyFee4262.spx"}';
    this.downloadVoice = '{"localId":"SXVOICE_e73a506ef84a91a8b30719043ca4360a"}';
...
var debugData = {
    ...
    config: function (params) {
        ...
        result.checkStringParametersForKeys(['appId', 'nonceStr', 'signature']);
        if (result.isOk) {
    ...
    configRequest: function (params) {
        ...
        if (result.isOk) {
            ...
            if (dataEnvironment.requestConfig.isValidatesSign) {
                dataEnvironment.requestConfig.appId = utils.getAvailableValue(params.appId, PrototypeString);
                dataEnvironment.requestConfig.appSecret = utils.getAvailableValue(params.appSecret, PrototypeString);
            }
    ...
    startRequest: function (obj) {
        ...
        if (result.isOk) {
            ...
            var requestParams = utils.getAvailableValue(params.requestParams, PrototypeObject);
    ...
    downloadImages: function (params) {
        ...
        if (result.isOk) {
            if (params.serverIds.length > MAX_VALUE_IMG_COUNT) {
                return resultMsg.getResultWithResCodeAndResMsg(RESCODE_TOO_MANY_IMAGES, ESMSG_TOO_MANY_IMAGES);
            }
    ...
    generateQRCode: function (params) {
        ...
        if (result.isOk) {
            ...
            var iconUrl = utils.getAvailableValue(params.iconUrl, PrototypeString);
            if (iconUrl) {
    ...
    alertMessage: function (params) {
        ...
        result.initWithParams(params);
        if (result.isOk) {
            var title = utils.getAvailableValue(params.title, PrototypeString);
            var message = utils.getAvailableValue(params.message, PrototypeString);
    ...
    checkIfUserFollowInstitution: function (params) {
        return JSON.parse(jsonData.checkIfUserFollowInstitution);
    },
    ...
    getUserInfo: function (params) {
        ...
        if (result.isOk) {
            params = params || {};
            var isGetDetailInfo = utils.getBoolValue(params.isGetDetailInfo, DEFAULT_BOOL_GET_DETAIL_USER_INFO);
            if (isGetDetailInfo) {
                return JSON.parse(jsonData.getDetailUserInfo);
            } else {
...
var utils = {
    getAvailableValue: function (value, prototype) {
    ...
    addSignToRequestParams: function (params) {
        var requestParams = {
            AppKey: dataEnvironment.requestConfig.appId,
    ...
    sign: function (params) {
        ...
        str = str.substr(0, str.length - 1) + dataEnvironment.requestConfig.appSecret;
